        ...
        <div class="col-lg-12">
            <form action="" name="searchform" id="searchform" method="GET" >
                <table class="table table-bordered" >
                    <tr><th><a href="{{request()->fullUrlWithQuery(["orderby"=>(request()->input('orderby')=="table_number asc")?"table_number desc":"table_number asc"])}}">TABLE NUMBER</th><th><a href="{{request()->fullUrlWithQuery(["orderby"=>(request()->input('orderby')=="seating asc")?"seating desc":"seating asc"])}}">SEATING</th><th><a href="{{request()->fullUrlWithQuery(["orderby"=>(request()->input('orderby')=="description asc")?"description desc":"description asc"])}}">DESCRIPTION</th><th><a href="{{request()->fullUrlWithQuery(["orderby"=>(request()->input('orderby')=="priority asc")?"priority desc":"priority asc"])}}">PRIORITY</th><th><a href="{{request()->fullUrlWithQuery(["orderby"=>(request()->input('orderby')=="duration asc")?"duration desc":"duration asc"])}}">DURATION</th><th>Actions</th>
                    </tr><tr><th><input class="form-control" type="text" name="table_number" value="{{request()->input('table_number')}}" ></th><th><input class="form-control" type="text" name="seating" value="{{request()->input('seating')}}" ></th><th><input class="form-control" type="text" name="description" value="{{request()->input('description')}}" ></th><th><input class="form-control" type="text" name="priority" value="{{request()->input('priority')}}" ></th><th><input class="form-control" type="text" name="duration" value="{{request()->input('duration')}}" ></th><th><input type="submit"  class="btn btn-primary"></th></tr>@foreach($model as $key=>$val)
                    <tr><td>{{$val->table_number}}</td><td>{{$val->seating}}</td><td>{{$val->description}}</td><td>{{$val->priority}}</td><td>{{$val->duration}}</td><td>
                            <a href="{{ url("admin/tables/edit",$val->id) }}" class="btn btn-warning btn-sm">Edit</a>
                            <a href="{{ url("admin/tables/show",$val->id) }}" class="btn btn-info btn-sm">Show</a>
                            <a onclick="return confirm('Do you really want to delete this record?');" href="{{ url("admin/tables/destroy",$val->id) }}" class="btn btn-danger btn-sm">Delete</a>
                        </td>
                    </tr>
                    @endforeach
                </table>
